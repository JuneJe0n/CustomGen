#!/usr/bin/env python
# custom_canny_face.py  (2025-07-09)
...
def main(ctrl, use_style, gpu_idx):
    ...
    edge_pil = Image.fromarray(cv2.cvtColor(edges, cv2.COLOR_GRAY2RGB)).resize(pose_im.size, Image.BILINEAR)
    edge_pil.save(OUTDIR/"edge.png")

    # ── 얼굴 bbox → mask
    info = max(face_det.get(face_cv), key=lambda d:(d['bbox'][2]-d['bbox'][0])*(d['bbox'][3]-d['bbox'][1]))
    w,h  = pose_im.size
    x1,y1,x2,y2 = map(int, info['bbox'])
    sx,sy = w/face_im.width, h/face_im.height
    x1,y1,x2,y2 = int(x1*sx), int(y1*sy), int(x2*sx), int(y2*sy)
    mask = np.zeros([h,w,3], np.uint8); mask[y1:y2, x1:x2] = 255
