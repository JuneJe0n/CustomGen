...
def main(use_style, gpu_idx):
    ...
    # Try different det_size values
    detection_sizes = [(1024, 1024), (640, 640), (320, 320)]
    face_det_prepared = False
    
    for det_size in detection_sizes:
        try:
            face_det.prepare(ctx_id=gpu_idx, det_size=det_size, det_thresh=0.3)
            print(f"🔍 Face detector prepared with det_size: {det_size}")
            face_det_prepared = True
            break
        except Exception as e:
            print(f"⚠️ Failed to prepare face detector with det_size {det_size}: {e}")
            continue
    
    if not face_det_prepared:
        print("❌ Failed to prepare face detector with any det_size")
        return
    ...
    if aligned_bbox is None:
        ...
        # Try different det_size for pose detection
        pose_faces = []
        for det_size in detection_sizes:
            face_det.prepare(ctx_id=gpu_idx, det_size=det_size, det_thresh=0.3)
            pose_faces = face_det.get(pose_cv)
            if pose_faces:
                print(f"✅ Pose face detected with det_size: {det_size}")
                break
            else:
                print(f"❌ No pose face detected with det_size: {det_size}")
        
        p_info = max(pose_faces, key=lambda d:(d['bbox'][2]-d['bbox'][0])*(d['bbox'][3]-d['bbox'][1]))
        ...
        # Try different det_size for face detection
        face_faces = []
        for det_size in detection_sizes:
            face_det.prepare(ctx_id=gpu_idx, det_size=det_size, det_thresh=0.3)
            face_faces = face_det.get(face_cv)
            if face_faces:
                print(f"✅ Source face detected with det_size: {det_size}")
                break
            else:
                print(f"❌ No source face detected with det_size: {det_size}")
        
        f_info = max(face_faces, key=lambda d:(d['bbox'][2]-d['bbox'][0])*(d['bbox'][3]-d['bbox'][1]))
