...
class IPAdapter:
    ...
    def get_image_embeds(self, pil_image=None, clip_image_embeds=None, content_prompt_embeds=None):
        ...
        image_prompt_embeds = self.image_proj_model(clip_image_embeds) # CA injection 위해 project
        uncond_image_prompt_embeds = self.image_proj_model(torch.zeros_like(clip_image_embeds)) # empty style token
        ...
        **kwargs,
        ...
        if neg_content_emb is None:
            if neg_content_prompt is not None:
                with torch.inference_mode():
                    (
                        prompt_embeds_, # torch.Size([1, 77, 2048]), token level emb
                        negative_prompt_embeds_,
                        pooled_prompt_embeds_, # torch.Size([1, 1280]), 전체 문장 level emb
        ...
        # image_prompt_embeds = img-content한것
        # uncond_image_prompt_embeds = empty style token
