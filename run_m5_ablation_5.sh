# Batch processing script for m5_ablation_5.py
# Usage: ./run_m5_ablation_5_batch.sh <face_folder> <pose_folder> <style_folder> [gpu_id]

set -e

# Check arguments
if [ $# -lt 3 ]; then
    echo "Usage: $0 <face_folder> <pose_folder> <style_folder> [gpu_id]"
    echo "Example: $0 ./faces ./poses ./styles 0"
    exit 1
fi
            ...
            if [[ -f "$pose_file" && "$pose_file" =~ \.(jpg|jpeg|png|bmp|tiff)$ ]]; then
                ...
                for style_file in "$STYLE_FOLDER"/*; do
                    if [[ -f "$style_file" && "$style_file" =~ \.(jpg|jpeg|png|bmp|tiff)$ ]]; then
                        ...
                        output_dir="/data2/jiyoon/custom/results/method5_ablation/ablation_5_batch/$outdir_name"
                        
                        echo "[$current_combination/$total_combinations] Processing: $outdir_name"
...
echo "Processed $current_combination combinations total"