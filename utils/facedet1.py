...
def main():
    ...
    for img_path in tqdm(image_files, desc="Extracting faces"):
        try:
            ...
            if not f_faces:
                print(f"No face detected in {img_path.name}, saving original image")
                no_face_detected.append(img_path.name)
                # Save original image when no face is detected
                output_path = output_folder / img_path.name
                face_im.save(output_path)
                success_count += 1
                continue
            ...
            print(f"Error processing {img_path.name}: {e}, saving original image")
            # Save original image when face extraction fails
            try:
                face_im = Image.open(img_path).convert("RGB")
                output_path = output_folder / img_path.name
                face_im.save(output_path)
                ...
                print(f"Failed to save original image {img_path.name}: {save_error}")
                failed_count += 1
    ...
    if no_face_detected:
        print(f"\nImages with no face detected ({len(no_face_detected)}):")
